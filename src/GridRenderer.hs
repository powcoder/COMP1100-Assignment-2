...
Functions to render a 'Grid' to a CodeWorld 'Picture', and to convert
...
-- | A type to represent transformations, which we will interpret into matrices and CodeWorld calls.
--
...
-- | Turn a 'Point' in CodeWorld screen space into a cell coordinate.
fromPoint :: Grid c -> Point -> Maybe (Int, Int)
fromPoint g@(Grid w h _) (x, y)
...
gridTransforms :: Grid c -> [Transform]
gridTransforms g =
  [ Scale (scaleFactor g) -- Shrink the grid to fit in the viewport
  , toCenter g -- Centre the grid around the origin
  ]

-- | Given a way to render one cell, render the grid.
renderGrid :: forall c . (c -> Picture) -> Grid c -> Picture
renderGrid f g@(Grid w h cells) = toCodeWorld
  (gridTransforms g)
  (CW.pictures (zipWith draw cells (allCoords w h)))
  where
    -- We flip the Y-axis here because our transformation DSL can't do
    -- reflection. We do all other transforms through
    -- 'gridTransfroms'.
    draw :: c -> GridCoord -> Picture
    draw c (x,y) = CW.translated (fromIntegral x) (fromIntegral (-y)) (f c)

-- | Compute the scaling to fit the grid in the screen. CodeWorld's
-- viewport is roughly 20 units tall (10..-10) and 30 units wide
-- (-15..15).
scaleFactor :: Grid c -> Double
scaleFactor (Grid w h _) = min (30 / w') (20 / h')
  where
    (w', h') = (fromIntegral w, fromIntegral h)

toCenter :: Grid c -> Transform
toCenter (Grid w h _) = Translate offsetX offsetY
  where
